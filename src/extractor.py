...
class TextExtractor:
    ...
    def _validate_rom(self, rom_data: bytes) -> None:
        ...
        if expected_size and len(rom_data) != expected_size:
            raise ValueError(
                f"ROM size mismatch: expected {expected_size}, got {len(rom_data)}"
            )
            ...
            if original_bytes:
                ...
                strings.append(
                    ExtractedString(
                        ...
                        pointer_address=table_address
                        + (i * 2),  # Assuming 16-bit pointers
    ...
    def _extract_auto_detect(self, rom_data: bytes) -> List[ExtractedString]:
        ...
        for i, candidate in enumerate(candidates):
            original_bytes = rom_data[
                candidate.address : candidate.address + candidate.length
            ]
                ...
                writer.writerow(
                    {
                        ...
                        "pointer_address": (
                            f"0x{string.pointer_address:04X}"
                            if string.pointer_address
                            else ""
                        ),
                    }
                )
