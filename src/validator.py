...
class ROMValidator:
    ...
    def validate_modified_rom(
        ...
        results.append(
            self._check_change_regions(original_data, modified_data, changed_regions)
        )
    ...
    def validate_translation_consistency(
        self, original_strings: List[str], translated_strings: List[str]
    ) -> List[ValidationResult]:
        ...
        results.append(
            self._check_format_preservation(original_strings, translated_strings)
        )

        # Check length constraints
        results.append(
            self._check_length_constraints(original_strings, translated_strings)
        )

        # Check for untranslated strings
        results.append(
            self._check_untranslated_strings(original_strings, translated_strings)
        )
    ...
    def _check_file_size(self, rom_data: bytes) -> ValidationResult:
        ...
        if actual_size == expected_size:
            return ValidationResult(
                "file_size", True, f"File size correct: {actual_size} bytes"
            )
        else:
            return ValidationResult(
                "file_size",
    ...
    def _check_crc32(self, rom_data: bytes) -> ValidationResult:
        ...
        if expected_crc is None:
            return ValidationResult(
                "crc32", True, "CRC32 check skipped (not configured)"
            )
        ...
        if actual_crc == expected_crc:
            return ValidationResult("crc32", True, f"CRC32 matches: {actual_crc:08X}")
        else:
            return ValidationResult(
                "crc32",
                False,
    ...
    def _check_nes_header(self, rom_data: bytes) -> ValidationResult:
        ...
        if len(rom_data) < 16:
            return ValidationResult(
                "nes_header", False, "ROM too small to contain NES header"
            )
        ...
        if rom_data[0:4] == b"NES\x1a":
            ...
            return ValidationResult(
                "nes_header", False, "Invalid or missing NES header signature"
            )
    ...
    def _check_known_patterns(self, rom_data: bytes) -> ValidationResult:
        ...
        if not known_patterns:
            return ValidationResult(
                "known_patterns", True, "No known patterns configured to check"
            )
        ...
        return ValidationResult(
            "known_patterns", True, f"All {len(known_patterns)} known patterns match"
        )
    ...
    def _check_size_consistency(
        ...
        if len(original_data) == len(modified_data):
            return ValidationResult(
                "size_consistency", True, "ROM size unchanged after modification"
            )
        else:
            return ValidationResult(
                "size_consistency",
    ...
    def _check_change_regions(
        ...
        if len(original_data) != len(modified_data):
            return ValidationResult(
                "change_regions", False, "Cannot check regions - ROM size changed"
            )
        ...
        for change_start, change_end in actual_changes:
            ...
            for exp_start, exp_end in expected_regions:
                if (
                    exp_start <= change_start < exp_end
                    and exp_start < change_end <= exp_end
                ):
        ...
        if not unexpected_changes:
            ...
            return ValidationResult(
                ...
                {
                    "unexpected_changes": [
                        f"0x{s:04X}-0x{e:04X}" for s, e in unexpected_changes
                    ]
                },
            )
    ...
    def _check_internal_checksums(self, rom_data: bytes) -> ValidationResult:
        ...
        if checksum_offset is None:
            return ValidationResult(
                "internal_checksums", True, "No internal checksums configured"
            )
    ...
    def _check_code_integrity(
        ...
        if len(original_data) < 0x8000:
            return ValidationResult(
                "code_integrity", True, "ROM too small for code integrity check"
            )
    ...
    def _check_string_count(
        self, original: List[str], translated: List[str]
    ) -> ValidationResult:
        """Check that string counts match."""
        if len(original) == len(translated):
            return ValidationResult(
                "string_count", True, f"String count matches: {len(original)} strings"
            )
        else:
            return ValidationResult(
                "string_count",
        ...
        for i, (orig, trans) in enumerate(zip(original, translated)):
            ...
            if orig_codes != trans_codes:
                format_issues.append(
                    f"String {i}: {orig_codes} -> {trans_codes} control codes"
                )

        if not format_issues:
            return ValidationResult(
                "format_preservation", True, "All formatting codes preserved"
            )
        else:
            return ValidationResult(
                "format_preservation",
    ...
    def _check_length_constraints(
        ...
        if not length_issues:
            return ValidationResult(
                "length_constraints", True, "All translations within length constraints"
            )
        else:
            return ValidationResult(
                "length_constraints",
    ...
    def _check_untranslated_strings(
        self, original: List[str], translated: List[str]
    ) -> ValidationResult:
        ...
        if not untranslated:
            return ValidationResult(
                "untranslated_strings", True, "All strings appear to be translated"
            )
        else:
            return ValidationResult(
