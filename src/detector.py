...
class TextDetector:
            ...
            for i, byte_val in enumerate(rom_data):
                if byte_val == terminator:
                    ...
                    if len(potential_string) >= self.min_string_length:
                        ...
                        if confidence > 0.4:
                            sample_text = self.encoding_table.decode_bytes(potential_string)
                            candidates.append(TextCandidate(
                                address=start,
    ...
    def _deduplicate_candidates(self, candidates: List[TextCandidate]) -> List[TextCandidate]:
    ...
    def analyze_rom(self, rom_path: str) -> Dict:
        ...
        return {
            ...
            'medium_confidence': len([c for c in candidates if 0.6 <= c.confidence <= 0.8]),
