...
class PointerUtils:
    ...
    @staticmethod
    def read_16bit_pointer(rom_data: bytes, address: int, little_endian: bool = True) -> int:
    ...
    @staticmethod
    def write_16bit_pointer(rom_data: bytearray, address: int, target: int, 
        ...
        if target > 0xFFFF:
            raise ValueError(f"Target address 0x{target:04X} too large for 16-bit pointer")
    ...
    @staticmethod
    def read_pointer_table(rom_data: bytes, table_address: int, count: int,
                          format_type: str = 'little_endian_16bit',
                          base_offset: int = 0) -> List[PointerInfo]:
        ...
        for i in range(count):
            ...
            target = PointerUtils.read_16bit_pointer(rom_data, ptr_address, little_endian)
            target += base_offset
            
            pointers.append(PointerInfo(
                address=ptr_address,
    ...
    @staticmethod
    def update_pointer_table(rom_data: bytearray, pointers: List[PointerInfo],
                           address_changes: Dict[int, int]) -> None:
    ...
    @staticmethod
    def find_pointer_references(rom_data: bytes, target_address: int,
                               search_range: Optional[Tuple[int, int]] = None) -> List[int]:
        ...
        for i in range(start, min(end, len(rom_data) - 1)):
            if (rom_data[i] == target_low and 
                rom_data[i + 1] == target_high):
                references.append(i)
        
        # Search for big endian pointers
        for i in range(start, min(end, len(rom_data) - 1)):
            if (rom_data[i] == target_high and 
                rom_data[i + 1] == target_low):
                references.append(i)
        
        return references
    
    @staticmethod
    def calculate_bank_address(address: int, bank_size: int = 0x4000,
                              bank_offset: int = 0x8000) -> Tuple[int, int]:
    ...
    @staticmethod
    def validate_pointer_chain(rom_data: bytes, pointers: List[PointerInfo]) -> List[str]:
        ...
        for i, pointer in enumerate(pointers):
            ...
            if pointer.address >= len(rom_data):
                issues.append(f"Pointer {i}: address 0x{pointer.address:04X} beyond ROM")
                continue
            
            # Check if target address is valid
            if pointer.target_address >= len(rom_data):
                issues.append(f"Pointer {i}: target 0x{pointer.target_address:04X} beyond ROM")
                continue
            ...
            if pointer.target_address < 0x100:
                issues.append(f"Pointer {i}: suspicious target 0x{pointer.target_address:04X}")
    ...
    @staticmethod
    def compact_pointer_targets(rom_data: bytearray, pointers: List[PointerInfo],
                               strings_data: List[bytes]) -> Dict[int, int]:
        ...
        for i, pointer in enumerate(sorted_pointers):
            ...
            if end_address > len(rom_data):
                raise ValueError(f"Not enough space to insert string at 0x{current_address:04X}")
